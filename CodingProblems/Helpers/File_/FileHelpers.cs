...
namespace CodingProblems.Helpers.File_
{
    ...
    internal static class FileHelpers
    {
        ...
        public static string WriteFileOfInts(int fileSizeInts, int minValue = 0, int maxValue = int.MaxValue - 1, string fileName = null)
        {
            // Use temp name if not specified.
            if (string.IsNullOrEmpty(fileName))
                fileName = Path.GetTempFileName();
            ...
            for (int i = minValue; i < fileSizeInts; i++)
            {
                // Write to buffer.
                buffer[i] = value;
